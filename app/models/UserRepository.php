...
class UserRepository
{
    ...
    public function signup($input)
    {
        ...
        $user_type = array_get($input, 'user_type');
        ...
        if($user_type=='adminrole'){
            $role = DB::table('roles')->where('name', '=', 'Admin')->pluck('id');
            $user->roles()->attach($role);
        }
        else if($user_type=='centerrole'){
            $role = DB::table('roles')->where('name', '=', 'Center_User')->pluck('id');
            $user->roles()->attach($role);
            $center_id=array_get($input, 'center_id');
            $user->center()->attach($center_id);
        }
        else if($user_type=='normalrole'){
            $role = DB::table('roles')->where('name', '=', 'Normal_User')->pluck('id');
            $user->roles()->attach($role);
        }
    ...
    public function settings_password($uname,$input){

        $user=Confide::getUserByEmailOrUsername($uname);

        if ($user) {
            $user->password              = $input['password'];
            $user->password_confirmation = $input['password_confirmation'];
            $user->date_start  = $input['start_date'];
            $user->date_end  = $input['end_date'];
            $result = $this->save($user);
            return true;
        }
