...
@section('content_options')
	...
	<div >
		<div class="heading pull-left" ><h3>Coordination Meetings </h3></div>
...
<br>		 
	  <div class="row">
	    ...
	    <div class="col-sm-3">
	    	<input id="date" type="text" class="form-control" value="{{$meeting['date']}}"  placeholder="">
	    </div>
	  </div>
<br>
	  <div class="row">
	    ...
	    <div class="col-sm-3">
	    	<input id="quater" type="text" class="form-control" value="{{$meeting['quater']}}"  placeholder="Enter Dates">
	    </div>
	  </div>
<br>
	  <div class="row">
	    ...
	    <div class="col-sm-3 ed">
	    	<ul id="rb" class="list-group">
	    		@foreach(json_decode($meeting['rightbased_org'],true) as $right_based)
	    		<li class="list-group-item" data-original-title="" title="" aria-describedby="popover738465">{{$right_based['name']}}<span style="display:none;">{{implode(',',$right_based['persons'])}}</span></li>
		    	@endforeach
		    </ul> 
	    </div>
	    <div class="col-sm-3">
...
<br>
	  <div class="row">
	    ...
	    <div class="col-sm-3 ed">
	    	...
	    	@foreach(json_decode($meeting['goveronment_deps'],true) as $tmp)
	    	<li class="list-group-item" data-original-title="" title="" aria-describedby="popover738465">{{$tmp['name']}}<span style="display:none;">{{implode(',',$tmp['persons'])}}</span></li>
	    	@endforeach
		    </ul> 
	    </div>
	    <div class="col-sm-3">
...
<br>
	  <div class="row">
	    ...
	    <div class="col-sm-3 ed">
	    	...
	    	@foreach(json_decode($meeting['social_activists'],true) as $tmp)
	    	<li class="list-group-item" data-original-title="" title="" aria-describedby="popover738465">{{$tmp['name']}}<span style="display:none;">{{implode(',',$tmp['persons'])}}</span></li>
	    	@endforeach
		    </ul> 
	    </div>
...
<br>
	    ...
	    <div class="col-sm-3 ed">
	    	...
	    	@foreach(json_decode($meeting['political_personalities'],true) as $tmp)
	    	<li class="list-group-item" data-original-title="" title="" aria-describedby="popover738465">{{$tmp['name']}}<span style="display:none;">{{implode(',',$tmp['persons'])}}</span></li>
	    	@endforeach
	    	</ul> 
	    </div>
...
<br>
	    ...
	    <div class="col-sm-3 ed">
	    	...
	    	@foreach(json_decode($meeting['district_bar'],true) as $tmp)
	    	<li class="list-group-item" data-original-title="" title="" aria-describedby="popover738465">{{$tmp['name']}}<span style="display:none;">{{implode(',',$tmp['persons'])}}</span></li>
	    	@endforeach
	    	</ul> 
	    </div>
...
$('#submit').on('click',function(evt){
	...
	$.ajax({
		...
		method:"post",
		data:{district:district,date:date,quater:quater,org:JSON.stringify(org), _method:"PATCH"},
